# small re-work of https://github.com/babichmorrowc/hull2spatial R package
...
#' @export
#' @examples
spLines2poly <- function(sp_lines, type = "sf", crs = 4326) {
  ...
  sp_polys <- SpatialPolygons(
    ...
    Srl = list(
      Polygons(
        ID = "1",
        srl = lapply(lines_slot@Lines, function(x) { Polygon(slot(x, "coords")) })
      )
    )
  )
  return(sp_polys)
...
ahull2poly <- function(hull, type = "sf", crs = 4326) {
  ...
  if (type == "sf") {
    return(st_as_sf(SpatialLines2SpatialPolygon))
  }
  if (type == "sp") {
