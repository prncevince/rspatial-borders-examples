...
ashape2poly <- function(ashape, type = "sf", crs = 4326) {
  ...
  if (type == "sf") {
    d_sf <- sf::st_sf(
      geometry = sf::st_sfc(sf::st_polygon(list(coords_subset)), crs = crs)
    )
    return(d_sf)
  }
  if (type == "sp") {
    ...
    spatial_ps <- SpatialPolygons(list(ps), proj4string = sp::CRS(sf::st_crs(crs)$proj4string))
    return(spatial_ps)
  }
}
